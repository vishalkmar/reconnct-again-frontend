...
import {
  ...
  ThumbsUp, ClipboardList, Clock, ChevronRight,
} from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-ink-muted">On-site quality checks assigned to you. Acknowledge, confirm you’re there, then give feedback.</p>
      </div>
              ...
              <div key={item.id} className="bg-white rounded-2xl shadow-soft overflow-hidden">
                <div className="p-5 flex flex-wrap items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className={`mt-2 inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${today ? 'bg-rose-50 text-rose-700' : 'bg-indigo-50 text-indigo-700'}`}>
                      <CalendarClock size={13} /> Visit: {qc.visitDate} at {qc.visitTime} {today && '· TODAY'}
                    </div>
                    {qc.instructions && (
                      <div className="mt-2 text-sm bg-surface-alt rounded-lg px-3 py-2 text-ink">
                        <span className="font-semibold">Instructions:</span> {qc.instructions}
                      </div>
                    )}
                  </div>
                  <Link to={`/team/review-queue/${item.id}`} className="text-xs font-semibold text-brand-dark inline-flex items-center gap-1 shrink-0">
                    View details <ChevronRight size={13} />
                  </Link>
                </div>

                {/* Stage-based action bar */}
                <div className="px-5 py-3 border-t border-slate-100 bg-surface-alt/40 flex items-center gap-2 flex-wrap">
                  ...
                  <div className="ml-auto flex items-center gap-2">
                    {stage === 'qc_assigned' && (
                      <button onClick={() => act(item.id, 'ack')} disabled={busyId === item.id}
                        className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-brand text-ink text-sm font-semibold hover:brightness-105 disabled:opacity-60">
                        <ThumbsUp size={15} /> Got it
                      </button>
                    )}
                    {stage === 'qc_acknowledged' && (
                      <button onClick={() => act(item.id, 'onsite')} disabled={busyId === item.id}
                        ...
                        <Navigation size={15} /> I’m at the location
                      </button>
                    )}
                    {stage === 'qc_onsite' && (
                      <button onClick={() => setFeedbackFor(item)} disabled={busyId === item.id}
                        ...
                        <ClipboardList size={15} /> Give Feedback
                      </button>
                    )}
                    {stage === 'qc_feedback' && (
                      <span className="inline-flex items-center gap-1.5 text-sm text-emerald-700 font-semibold"><CheckCircle2 size={16} /> Feedback submitted — awaiting Center Ops</span>
                    )}
                  </div>
                </div>
              </div>
                ...
                <div className="flex gap-2">
                  {[['Yes', true], ['No', false]].map(([lbl, val]) => (
                    <button key={lbl} type="button" onClick={() => set(f.key, val)}
                      className={`px-4 py-1.5 rounded-lg text-sm font-semibold border ${values[f.key] === val ? (val ? 'bg-emerald-500 text-white border-emerald-500' : 'bg-rose-500 text-white border-rose-500') : 'border-gray-200 text-ink-muted hover:bg-surface-alt'}`}>
                      {lbl}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {f.options.map((opt) => (
                    <button key={opt} type="button" onClick={() => set(f.key, opt)}
                      className={`px-3 py-1.5 rounded-lg text-sm font-semibold border capitalize ${values[f.key] === opt ? 'bg-brand text-ink border-brand' : 'border-gray-200 text-ink-muted hover:bg-surface-alt'}`}>
                      {opt.replace('_', ' ')}
                    </button>
                  ))}
                </div>
