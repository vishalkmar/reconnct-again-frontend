...
import {
  TrendingUp, Plus, Search, Loader2, Check, X, Pencil, Trash2, Pause, Play,
  Globe, Layers, Users, Sparkles, BarChart3, RotateCcw, IndianRupee, Percent, ChevronRight, AlertTriangle,
} from 'lucide-react';
...
/*
  Markup Management — the ONE place a markup is set.

  An admin adds a rule for a scope (everything / broad categories / "Who is
  this for" audiences / specific listings) and every matching experience picks
  it up automatically — including experiences added later. The go-live pricing
  screen only displays the result.

  When several rules match one experience (its category says 10%, its audience
  says 4%, it has its own 5%) the LATEST APPLIED rule wins. That's shown
  explicitly in the rules table so it never has to be guessed.
*/
...
const showValue = (m) => (!m ? '—' : (m.type === 'fixed' ? rupee(m.value) : `${Number(m.value)}%`));
...
export default function MarkupManagementPage() {
  ...
  const resync = async () => {
    setResyncing(true);
    try {
      const { data } = await api.post('/admin/pricing-setup/markup/resync');
  ...
  const toggle = async (rule) => {
    try {
      await api.patch(`/admin/pricing-setup/markup/rules/${rule.id}/toggle`);
      load();
    } catch (err) { toast.error(err.response?.data?.message || 'Failed'); }
  };

  const remove = async (rule) => {
    if (!window.confirm('Remove this markup rule? Every experience it currently drives falls back to the next most recent rule.')) return;
    try {
      const { data } = await api.delete(`/admin/pricing-setup/markup/rules/${rule.id}`);
      toast.success(data.message || 'Removed');
      load();
    } catch (err) { toast.error(err.response?.data?.message || 'Failed'); }
  };

  const withMarkup = effective.filter((e) => e.markup && Number(e.markup.value) > 0);
  const liveWithout = effective.filter((e) => e.live && (!e.markup || !Number(e.markup.value)));

  return (
    <PricingSetupShell
      title="Markup Management"
      subtitle="Set the markup once here and every matching experience picks it up automatically — the go-live screen only shows it."
    >
      {/* Actions + at-a-glance */}
      <div className="flex flex-wrap items-center gap-2 mb-5">
        ...
        <Link to="/admin/pricing-setup/markup/analysis"
      ...
      <div className="grid sm:grid-cols-3 gap-3 mb-6">
        ...
        <Stat label="Experiences with markup" value={withMarkup.length} icon={Check} tint="text-emerald-600 bg-emerald-50" />
        <Stat label="Live listings without markup" value={liveWithout.length} icon={AlertTriangle} tint="text-amber-600 bg-amber-50" />
      </div>
...
function MarkupForm({ targets, rules, rule, onClose, onSaved }) {
  ...
  const chooseScope = (key) => {
    setScope(key);
    setPicked([]);
    setApplied(key === 'all'); // "To All" needs no target picking
  };

  const togglePick = (id) => setPicked((p) => (p.includes(id) ? p.filter((x) => x !== id) : [...p, id]));

  const save = async () => {
    if (!scope) return toast.error('Choose where this markup applies');
    if (scope !== 'all' && !picked.length) return toast.error('Select at least one target');
    if (!Number(value)) return toast.error('Enter a markup value');
    setSaving(true);
    try {
      const body = { scope, targetIds: scope === 'all' ? [] : picked, type, value: Number(value), note };
      const { data } = editing
        ? await api.put(`/admin/pricing-setup/markup/rules/${rule.id}`, body)
        : await api.post('/admin/pricing-setup/markup/rules', body);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-soft p-5 mb-6 border border-brand/20">
      <div className="flex items-start justify-between mb-4">
        <div>
          <h2 className="font-display font-bold text-lg">{editing ? 'Edit markup' : 'Add markup'}</h2>
          ...
          {SCOPES.map((s) => (
            <button key={s.key} type="button" onClick={() => chooseScope(s.key)}
              className={`text-left rounded-xl border p-3 transition ${
                scope === s.key ? 'border-brand bg-brand/5 shadow-soft' : 'border-gray-200 hover:border-brand/40'
              }`}>
            ...
            {options.map((o) => (
              <label key={o.id} className="flex items-center gap-3 px-3 py-2 cursor-pointer hover:bg-surface-alt/60">
                <input type="checkbox" checked={picked.includes(o.id)} onChange={() => togglePick(o.id)}
                  className="w-4 h-4 accent-[color:var(--brand,#e0a82e)]" />
      ...
      {applied && scope && (
        <div className="rounded-xl bg-surface-alt/60 p-4">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div>
              ...
              <select className="input" value={type} onChange={(e) => setType(e.target.value)}>
                ...
                <option value="fixed">Flat (fixed amount ₹) markup</option>
              </select>
            </div>
            <div>
          ...
          <div className="mt-3">
            ...
            <input className="input" placeholder="e.g. Festive season margin" value={note} onChange={(e) => setNote(e.target.value)} />
          </div>
  ...
  return (
    <div className="bg-white rounded-2xl shadow-soft overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100">
        <h2 className="font-semibold">Markup rules</h2>
        ...
        <table className="w-full text-sm">
          <thead className="bg-surface-alt/60 text-[11px] uppercase tracking-wide text-ink-muted">
            <tr>
              ...
              <th className="text-left font-semibold px-3 py-2.5">Markup</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {rules.map((r) => {
              ...
              return (
                <tr key={r.id} className={r.isActive ? '' : 'opacity-55'}>
                  ...
                  <td className="px-3 py-3 font-bold text-ink whitespace-nowrap">{showValue(r)}</td>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button onClick={() => onEdit(r)} title="Edit"
                        className="p-1.5 rounded-lg text-ink-muted hover:bg-surface-alt hover:text-brand"><Pencil size={15} /></button>
                      <button onClick={() => onRemove(r)} title="Remove"
                        className="p-1.5 rounded-lg text-ink-muted hover:bg-rose-50 hover:text-rose-600"><Trash2 size={15} /></button>
                    </div>
                  </td>
                </tr>
              );
            ...
            {rules.length === 0 && (
              <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-ink-muted">
                No markup set yet. Add one and every matching experience picks it up automatically.
              </td></tr>
            )}
          </tbody>
        </table>
    ...
    <div className="bg-white rounded-2xl shadow-soft overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100 flex flex-wrap items-center gap-3">
        <div className="flex-1 min-w-[12rem]">
          <h2 className="font-semibold">Effective markup per experience</h2>
          <p className="text-xs text-ink-muted">The resolved value each listing carries right now — this is exactly what the go-live screen shows.</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-surface-alt/60 text-[11px] uppercase tracking-wide text-ink-muted sticky top-0">
            <tr>
              ...
              <th className="text-right font-semibold px-3 py-2.5">B2B base</th>
              <th className="text-left font-semibold px-3 py-2.5">Markup</th>
              <th className="text-right font-semibold px-3 py-2.5">+ Amount</th>
              <th className="text-right font-semibold px-5 py-2.5">Base after markup</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {rows.map((i) => (
              <tr key={i.id}>
                ...
                <td className="px-3 py-2.5 text-right text-ink-muted whitespace-nowrap">{rupee(i.basePrice)}</td>
                <td className="px-3 py-2.5">
                  {i.markup && Number(i.markup.value) > 0 ? (
                    <div>
                      <span className="font-semibold text-ink">{showValue(i.markup)}</span>
                      <div className="text-[11px] text-ink-muted truncate max-w-[14rem]">{i.ruleLabel}</div>
                    </div>
                  ) : <span className="text-ink-muted">—</span>}
                </td>
                <td className="px-3 py-2.5 text-right whitespace-nowrap text-emerald-600 font-medium">
                  {i.markupAmount ? `+ ${rupee(i.markupAmount)}` : '—'}
                </td>
                <td className="px-5 py-2.5 text-right font-bold text-ink whitespace-nowrap">{rupee(i.finalBase)}</td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-ink-muted">Nothing matches.</td></tr>
            )}
          </tbody>
        </table>
      </div>
