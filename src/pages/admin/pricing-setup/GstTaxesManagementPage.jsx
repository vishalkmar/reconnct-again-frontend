...
import {
  ...
  Globe, Layers, Users, Sparkles, BarChart3, ChevronRight, AlertTriangle, ShieldCheck,
} from 'lucide-react';
...
// ─── Add / edit a rule ──────────────────────────────────────────────────────
function GstForm({ targets, rules, rule, onClose, onSaved }) {
  const editing = !!rule;
    ...
    <div className="bg-white rounded-2xl shadow-soft p-5 mb-6 border border-brand/20">
      ...
      {scope === 'all' && currentGlobal && (!editing || currentGlobal.id !== rule.id) && (
        <div className="flex items-start gap-2 rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 text-xs text-amber-800 mb-5">
          <AlertTriangle size={15} className="mt-0.5 shrink-0" />
          <span>
            There is already a platform-wide GST of <strong>{Number(currentGlobal.rate) > 0 ? `${Number(currentGlobal.rate)}%` : 'Off'}</strong>. Saving
