...
/*
  Discount analysis — what the coupons actually gave away.

  Every figure comes from `booking.couponDiscountPaise` on paid bookings, so
  it's money genuinely taken off, never a projection. Single-hue charts: the
  discount is a magnitude, darker = more given away.
*/

const HUE = ['#9f1239', '#be123c', '#e11d48', '#f43f5e', '#fb7185', '#fda4af'];
...
export default function DiscountAnalyticsPage() {
  ...
  const [f, setF] = useState({ code: '', categoryId: '', audienceId: '', experienceId: '', city: '' });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get('/admin/pricing-setup/discount/analytics', { params });
      setData(res.data?.data || null);
    } catch (err) {
      toast.error(err.response?.status === 404
        ? 'Discount analytics isn’t live on this server yet — deploy the backend carrying Discount Management.'
        : (err.response?.data?.message || 'Could not load discount analytics'));
  ...
  const clearFilters = () => setF({ code: '', categoryId: '', audienceId: '', experienceId: '', city: '' });
  const anyFilter = Object.values(f).some((v) => v !== '' && v != null);

  const exportCsv = () => {
    const rows = data?.bookings || [];
    if (!rows.length) return toast.error('Nothing to export');
    const head = ['Booking', 'Coupon', 'Guest', 'Experience', 'City', 'Paid on', 'Guests', 'Discount given', 'Customer paid'];
    const body = rows.map((b) => [
      b.code, b.couponCode || '', b.guest, b.experience, b.city || '',
      new Date(b.date).toLocaleDateString('en-IN'), b.guests, b.discount, b.revenue,
    ]);
    const csv = [head, ...body].map((r) => r.map((c) => `"${String(c ?? '').replace(/"/g, '""')}"`).join(',')).join('\n');
    const url = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8;' }));
    const a = document.createElement('a');
    a.href = url; a.download = `discount-${data.range.start}_${data.range.end}.csv`; a.click();
  ...
  return (
    <PricingSetupShell
      ...
      subtitle="What the coupons gave away — paid bookings only, counted on the day the money landed."
    >
      <div className="flex flex-wrap items-center gap-2 mb-5">
        <Link to="/admin/pricing-setup/discount"
      ...
      <div className="bg-white rounded-2xl shadow-soft p-4 mb-5">
        ...
        <div className="flex flex-wrap items-end gap-3 mt-4 pt-4 border-t border-gray-100">
          <Select label="Coupon code" value={f.code} onChange={(v) => setFilter('code', v)}
            options={(data?.filters?.codes || []).map((c) => ({ value: c, label: c }))} />
      ...
      {loading ? (
        ...
        <div className="space-y-5">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <Kpi label="Total discount given" value={rupee(data.totals.discount)} icon={IndianRupee} delta={data.delta}
              sub={`vs ${rupee(data.previous.discount)} in the previous period`} hero />
            <Kpi label="Bookings with a coupon" value={data.totals.redeemedBookings} icon={Ticket}
              sub={`of ${data.totals.bookings} paid bookings`} />
            <Kpi label="Avg discount" value={rupee(data.totals.avgDiscount)} icon={CalendarDays}
              sub="per redeeming booking" />
            <Kpi label="Redemption rate" value={`${data.totals.redemptionRate}%`} icon={Percent}
              sub={`${data.totals.discountRate}% of gross given away`} />
          </div>

          <Card title="Discount given over time" note={`Grouped by ${iv}. Paid bookings only.`}>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={trend} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                ...
                <Tooltip cursor={{ fill: '#e11d4814' }} content={<TrendTip />} />
                <Bar dataKey="discount" fill={TREND} radius={[4, 4, 0, 0]} maxBarSize={44} />
              </BarChart>
            </ResponsiveContainer>
          </Card>

          <div className="grid lg:grid-cols-2 gap-5">
            <RankCard title="Discount by coupon" rows={data.byCoupon} empty="No coupon was redeemed in this period." />
            <RankCard title="Discount by activity" rows={data.byActivity} empty="No redemptions yet." />
            <RankCard title="Discount by broad category" rows={data.byCategory} empty="No categorised redemptions yet." />
            <RankCard title="Discount by city" rows={data.byCity} empty="No city data yet." />
          </div>

          <Card title="Redeemed bookings" note={`${data.bookings.length} most recent booking(s) that used a coupon.`}>
              ...
              <table className="w-full text-sm">
                <thead className="bg-surface-alt/60 text-[11px] uppercase tracking-wide text-ink-muted sticky top-0">
                  <tr>
                    ...
                    <th className="text-left font-semibold px-3 py-2.5">Coupon</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {data.bookings.map((b) => (
                    <tr key={b.id}>
                      ...
                      <td className="px-3 py-2.5 font-mono text-xs font-bold text-ink">{b.couponCode || '—'}</td>
                      <td className="px-3 py-2.5">
                      ...
                      <td className="px-3 py-2.5 text-right font-bold text-rose-600 whitespace-nowrap">− {rupee(b.discount)}</td>
                      <td className="px-3 py-2.5 text-right text-ink-muted whitespace-nowrap">{rupee(b.revenue)}</td>
                    </tr>
                  ))}
                  {data.bookings.length === 0 && (
                    <tr><td colSpan={6} className="px-3 py-12 text-center text-sm text-ink-muted">No coupon was redeemed in this period.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
...
function Kpi({ label, value, icon: Icon, sub, delta, hero }) {
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-soft p-4 ${hero ? 'ring-1 ring-rose-500/20' : ''}`}>
      ...
      <div className="flex items-center gap-1.5 mt-1">
        {delta != null && (
          <span className={`inline-flex items-center gap-0.5 text-[11px] font-bold ${up ? 'text-rose-600' : 'text-emerald-600'}`}>
      ...
      {n === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={Math.max(160, n * 34)}>
          <BarChart data={top} layout="vertical" margin={{ top: 0, right: 16, left: 0, bottom: 0 }}>
            ...
            <Tooltip cursor={{ fill: '#e11d4814' }} content={<RankTip />} />
            <Bar dataKey="discount" radius={[0, 4, 4, 0]} maxBarSize={22}>
...
function TrendTip({ active, payload, label }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-gray-100 px-3 py-2 text-xs">
      ...
      <Line k="Discount given" v={rupee(d.discount)} strong />
