...
// Landing page for the emailed supplier reset link:
//   /supplier/reset-password?token=…&email=…
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-brand-light/20 via-white to-wellness-light/20 px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-ink-muted mt-1">{email ? `for ${email}` : 'Supplier account'}</p>
        </div>
        ...
        {badLink ? (
          <div className="bg-white rounded-2xl shadow-card p-8 text-center space-y-4">
            ...
            <button type="button" onClick={() => navigate('/supplier/login')} className="btn-primary w-full">Back to sign in</button>
          </div>
        ) : (
          <form onSubmit={submit} className="bg-white rounded-2xl shadow-card p-8 space-y-5">
            ...
            <button type="button" onClick={() => navigate('/supplier/login')} className="w-full text-center text-sm text-ink-muted hover:text-brand">← Back to sign in</button>
          </form>
        )}
      </div>
    </div>
