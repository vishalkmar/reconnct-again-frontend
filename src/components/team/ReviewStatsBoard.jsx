...
function StatTile({ icon: Icon, label, value, tone = 'slate', to }) {
  const inner = (
    <>
  ...
  if (to) return <Link to={to} className="bg-white rounded-2xl shadow-soft p-4 flex items-center gap-3 hover:shadow-lg transition">{inner}</Link>;
  return <div className="bg-white rounded-2xl shadow-soft p-4 flex items-center gap-3">{inner}</div>;
}
...
// The submitter's board — matches the My Experiences tabs, tiles are clickable
// and deep-link straight to the matching tab.
const MINE_TILES = [
...
function MineBoard() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      toast.error(e.response?.data?.message || 'Could not load your experiences');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const objectionItems = items.filter((i) => i.tab === 'in_queue' && i.objectionCount > 0);

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-display font-bold text-lg">My experiences</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {MINE_TILES.map((t) => (
          <StatTile key={t.key} icon={t.icon} label={t.label} value={counts[t.key] || 0} tone={t.tone} to={`/team/experiences?tab=${t.key}`} />
        ))}
      </div>

      <Panel title="Needs your action — objections to fix" count={objectionItems.length} tone="bg-rose-100 text-rose-700">
        {objectionItems.length === 0 ? <EmptyRow label="Nothing to fix right now 🎉" />
          : objectionItems.map((it) => (
